...
return static function (): array {
    return [
        ...
        'payment_methods.common_payment_processor' => new Factory(['embedded_payment.misconfiguration_detector', 'list_session.manager', 'inpsyde_payoneer_api.update_command_factory', 'checkout.security_token_generator', 'checkout.order.security_header_field_name', 'payment_methods.order.transaction_id_field_name', 'checkout.session_hash_key', 'payment_methods.transaction_url_template_field_name', 'payoneer_settings.merchant_id_field_name', 'payoneer_settings.merchant', 'payment_methods.order.awaiting_webhook_field_name'], static function (MisconfigurationDetectorInterface $misconfigurationDetector, ListSessionProvider $sessionProvider, WcOrderBasedUpdateCommandFactoryInterface $updateCommandFactory, TokenGeneratorInterface $tokenGenerator, string $tokenKey, string $transactionIdFieldName, string $sessionHashKey, string $transactionUrlTemplateFieldName, string $merchantIdFieldName, MerchantInterface $merchant, string $awaitingWebhookFieldName) {
            return new PayoneerCommonPaymentProcessor($misconfigurationDetector, $sessionProvider, $updateCommandFactory, $tokenGenerator, $tokenKey, $transactionIdFieldName, $sessionHashKey, $transactionUrlTemplateFieldName, $merchantIdFieldName, $merchant, $awaitingWebhookFieldName);
        }),
        // Refunds.
